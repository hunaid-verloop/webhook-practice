...
def find_available_slots(date_requested, duration_requested_minutes):
    ...
    for index in m_slots_i:
        start_hrs = morning_start + (index * default_duration)//60
        ...
        ats.append((start_hrs, start_mins, end_hrs, end_mins))

    for index in a_slots_i:
        start_hrs = afternoon_start + (index * default_duration)//60
        start_mins = (index * default_duration)%60
        end_hrs = start_hrs + (duration_requested_minutes)//60
        end_mins = start_mins + (duration_requested_minutes)%60
        carry = end_mins//60
...
def get_dates():
    ...
    return jsonify({
        "dates_around_requested_date": dates_around_requested_date
    })
...
def get_available_slots():
    ...
    for ts in available_timeslots:
        ...
        ats.append(f"{ts[0]}:{start_minutes}-{ts[2]}:{end_minutes}")
